<?php
	...
	$IDU=$_POST['Usuario'];
	$IDN=$_POST['Negocio'];
	$q1='select nombre_negocio from negocio where id_negocio='.$IDN.';';
	...
	if (isset($_POST['accion'])) {
		...
		if($BAN==0){
			...
			$sql="delete from favorito where id_usuario=".$IDU." AND id_negocio=".$IDN.";";
		}
	}else{
		if(isset($_POST['calificacion'])) {
			$CALIF=$_POST['calificacion'];
			$q1='select calificacion from calificacion where id_usuario='.$IDU.' and id_negocio='.$IDN.';';
			$result1=mysqli_query ($con,$q1);
